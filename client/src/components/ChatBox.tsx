...
const ChatBox: React.FC<ChatBoxProps> = (props) => {
    ...
    const [arrivalMessage, setArrivalMessage] = useState<MessageReturnType>({} as MessageReturnType);
    const [isTyping, setIsTyping] = useState<TypingType>({} as TypingType)

    useEffect(() => {

        socket.on("msg-receive", (data: { from: string, to: string, msg: string }) => {
        ...
        <div className="w-[320px] h-full mx-2 bg-white shadow-2xl rounded-lg flex flex-col">
            ...
            <div className="p-2 flex flex-row items-center justify-between rounded-lg rounded-b-none shadow-md">
                ...
                <div className="flex flex-row items-center">
                    ...
                    <div className="cursor-pointer" onClick={() => removeChatBox(user.userId, 'active')}>
                        <IoMdClose size={22} />
                    </div>
                </div>
            </div>

            <MessageBox messages={messages} friendId={currentFriend._id} isTyping={isTyping} />

            <ChatInput friendId={currentFriend._id} messages={messages} setMessages={setMessages} />
        </div >
    )
}
